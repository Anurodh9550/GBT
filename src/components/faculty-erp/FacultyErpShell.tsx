...
import { facultyErpGroups, facultyErpNav } from "@/lib/faculty-erp-nav";
import { siteConfig } from "@/lib/site-config";

const SESSION_KEY = "gbt-faculty-session";
...
export function getFacultySession(): FacultySession | null {
  ...
  try {
    ...
    return raw ? (JSON.parse(raw) as FacultySession) : null;
  } catch {
    return null;
  }
}

export function setFacultySession(session: FacultySession) {
        ...
        <ul className="bg-white pb-2">
          {items.map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className={`block py-2 pl-8 pr-4 text-sm transition hover:text-brand-green ${
                  pathname === item.href ? "font-semibold text-brand-green" : "text-slate-600"
                }`}
              >
                {item.label}
              </Link>
...
export default function FacultyErpShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const current = getFacultySession();
    if (!current) {
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      <header className="sticky top-0 z-40 flex h-14 items-center justify-between bg-brand-green px-3 text-white shadow-md sm:px-4">
        ...
        <div className="relative">
          ...
          {menuOpen && (
            <div className="absolute right-0 mt-2 w-48 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 text-sm text-slate-700 shadow-lg">
              <Link href="/faculty-erp/my-profile" className="block px-4 py-2 hover:bg-slate-50" onClick={() => setMenuOpen(false)}>
                My Profile
              </Link>
              <Link href="/faculty-erp/change-password" className="block px-4 py-2 hover:bg-slate-50" onClick={() => setMenuOpen(false)}>
                Change Password
              </Link>
      ...
      <div className="flex">
        ...
        >
          <div className="border-b border-slate-200 bg-slate-50 p-4 text-center">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-brand-green/10 font-serif text-xl font-bold text-brand-green">
              {session.name
                .split(" ")
                .map((part) => part[0])
                .join("")
            ...
            <p className="mt-1 text-xs text-slate-500">{session.employeeId}</p>
          ...
          <nav className="max-h-[calc(100vh-14rem)] overflow-y-auto">
            {facultyErpNav.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center gap-3 border-b border-slate-100 px-4 py-3 text-sm font-medium ${
                  pathname === item.href ? "border-l-2 border-l-brand-green bg-brand-green/5 text-brand-green" : "text-slate-700 hover:bg-slate-50"
                }`}
              >
            ...
            {facultyErpGroups.map((group) => (
              <NavGroup
                key={group.label}
                label={group.label}
                items={group.items}
                pathname={pathname}
                defaultOpen={group.label === "TEACHING"}
              />
            ))}
          </nav>
        </aside>
